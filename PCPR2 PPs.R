...
profilespp <- read.csv("Urinary polyphenols_EPIC.csv", header=T)
X_DataMatrix <- profilespp[, 6:41]
napositions <- which(is.na(X_DataMatrix), arr.ind=TRUE)
X_DataMatrix[napositions] <- apply(X_DataMatrix, 2, function(x) median(x, na.rm=T))[napositions[ ,2]]
Z_InterestFactors <- profilespp[, 2:5] #Y variables
...
Z_InterestFactors$country <- as.factor(Z_InterestFactors$country)
Z_InterestFactors$centre <- as.factor(Z_InterestFactors$centre)
Z_InterestFactors$sex <- as.factor(Z_InterestFactors$sex)
Z_InterestFactors$bmi <- Z_InterestFactors$bmi
DataCol <- ncol(Data)
...
for (i in 1:pc_n){                                        
  ...
  Model <- lm((pc_data_matrix ~ centre + sex + bmi), Data[y:(((i-1)*Z_InterestFactorsRowN)+Z_InterestFactorsRowN),])
